...
cp front-a11y/SKILL.md front-a11y/front-a11y-*.md ~/.claude/skills/front-a11y/
```
...
| Argument | Effect |
|---|---|
| *(none)* | Audit mode: analyse the file and produce a structured report of all accessibility issues found. The file is not modified. |
| `fix` | Fix mode: apply all automatically-fixable issues directly to the file, then report what was fixed and what requires manual attention. |
...
Issues are grouped by three severity levels:

### 🔴 Critical

Blocks assistive technology users entirely:

- Missing `alt` attributes on meaningful images
- Form inputs with no associated label
- Interactive elements with no accessible name (icon-only buttons, empty links)
- Missing `lang` attribute on `<html>`

These issues prevent screen reader users from understanding or operating the page.

### 🟠 Major

Significantly degrades the accessible experience:

- Positive `tabindex` values that break natural focus order
- Non-interactive elements (`<div>`, `<span>`) with `onclick` but no keyboard support
- `<iframe>` elements without a `title` attribute
- Decorative SVGs not hidden from assistive technology

These issues do not fully block usage but create real friction for keyboard and screen reader users.

### 🟡 Minor

Reduces quality or consistency but has a workaround:

- Generic link text ("Read more", "Click here") without context
- Links opening in a new tab without warning the user
- Images with a missing `alt` where the context suggests a meaningful description exists

These are best fixed alongside the Critical and Major issues to achieve complete WCAG 2.1 AA compliance.
...
| Issue | Fix applied |
|---|---|
| Missing `lang` on `<html>` | Adds `lang="en"` |
| Decorative SVG not hidden | Adds `aria-hidden="true"` |
| `tabindex` > 0 | Replaced with `tabindex="0"` |
| `<button tabindex>` (natively focusable) | Removes `tabindex` entirely |

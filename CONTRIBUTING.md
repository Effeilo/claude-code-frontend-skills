...
This repository includes a validation script that acts as a lightweight quality gate for the project structure and documentation.
...
- every documented skill directory includes `SKILL.md`, `README.md`, and `examples/`
- `install.sh` and `install.ps1` list the same skills as the repository
- the example files mentioned in each skill README actually exist
- the repository uses local logo assets instead of external hotlinks

The same validation also runs automatically in GitHub Actions on each push and pull request.
